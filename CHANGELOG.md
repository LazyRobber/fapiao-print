...
- **印章/文字边缘被裁掉一点点**：内容边缘经「抗锯齿 + JPEG」后会有一圈 253~254 的淡像素，原阈值 252 把它判为白 → 检测边界内缩；且向外留边距（pad）只有 5px（≈0.42mm）。pad 改为**分方向**：左/上/下 **12px**（≈1mm）、右侧 **28px**（≈2.4mm，发票右侧常有「下载次数」「密码区」浅色小字，多留更安全）
...
| pad 精确 | 主体上边界 150 → 裁剪上边界 138（= 150−12）；左边界 50 → 38 |
