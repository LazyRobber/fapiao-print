...
- **浅灰细字被当白边裁掉**：发票右侧「下载次数：1」是 250 上下的浅灰细字，原 245 阈值把它算作白边（合成图实测复现：灰度 250/252 直接裁到主体边界，裁剪后即"消失"）。阈值提到 252，并要求一行/列**至少 2 个非白像素**才算「有内容」以抑制照片/JPEG 的孤立浅色噪点（纯白底 JPEG 噪点实测 255，放宽阈值无副作用）
- **上下白边裁不掉**：阈值放宽后，页面上下边缘的浅灰细线（248~251、1~2px 厚）也被当成内容顶住边界（实测上下白边 896/1203 → 296/3199）。两者灰度重叠、只能按**厚度**区分：新增 `HARD_THRESHOLD=200`（更暗的像素无条件算内容，黑字黑线行为不变），200~251 的浅灰像素需在垂直/水平方向**连续 ≥4 像素**才算真内容 —— 浅灰文字/印章够厚会保留，边缘细线与 JPEG 振铃太薄被忽略
...
| 浅灰细字 / 深色细线行为不变 | 250 灰度细字（约 30px 厚）保留；2px 黑线仍保留（深色内容无需厚度检验） |
