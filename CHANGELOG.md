...
## v2.6.3 — 发票类型识别修复（普通发票不再显示成增值税发票）
...
- **汇总表「发票类型」把所有普通发票显示成「增值税发票」**（issue #35）：这一列此前不是识别结果 —— 除 XML 数电票外一律返回写死的兜底字符串「增值税发票」，普通发票和专用发票在列里长得完全一样；而「增值税发票」在会计语境里常被读作专票，于是表现为「所有普通发票都被识别成专用发票」。现在改为单一真源 `resolveInvoiceType(f)`：分类标记（通行费 / 车票 / 非税票据）→ 结构化类型 → 兜底「发票」，不再编造类型
  - **专票 / 普票归一化**：新增 `normalizeInvoiceType()` —— 「电子发票(普通发票)」「增值税普通发票」「普通发票」统一显示为 **普票**，含「专用」的显示为 **专票**；汇总表、列表缩略图徽章、右键「复制发票信息」、汇总表重命名、CSV 导出共用同一口径
  - **OFD 数电票的类型被丢弃**：`parse_ofd` 早已解析出 `invoiceType`，但展示层只认 `_xmlInvoice`（仅 XML 为 true），OFD 的解析结果从未显示过。现在只要有类型信息就生效（列表徽章同理，OFD 此前只能显示「ofd」）
  - **OCR / PDF 文字层补专普识别**：此前整条前链路不区分专普，扫描件与 PDF 永远拿不到类型信息。新增 `_detectVatSubtype()`：票头标题区（`ny < 0.18`）优先、全文兜底，识别结果写回 `fileObj.invoiceType`（已有结构化类型时不覆盖）
...
| JS 函数级用例 24 例（归一化 / 真源优先级 / 徽章取值 / 票头优先 / 专普互斥） | 全部通过 |
