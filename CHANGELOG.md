...
- **汇总表「发票类型」把所有普通发票显示成「增值税发票」**（issue #35）：这一列此前不是识别结果 —— 除 XML 数电票外一律返回写死的兜底字符串「增值税发票」，普通发票和专用发票在列里长得完全一样；而「增值税发票」在会计语境里常被读作专票，于是表现为「所有普通发票都被识别成专用发票」。现在改为单一真源 `resolveInvoiceType(f)`：分类标记（通行费 / 车票 / 非税票据）→ 结构化类型 → 兜底「发票」，不再编造类型
  ...
  - **文字层坐标失效的 PDF 也能判出专普**：部分数电票 PDF 的文字层坐标解析失败（实测样本：整页 894 字全部 y≈0 被并成 1 行、词按 x 重排），按词序拼接会把「电子发票（增值税专用发票）」的连续标题打散导致判定失败。现增加内容流顺序的原始文本作为中间兜底（标题在写入顺序里是连续的），三层判定：票头区 → 原始全文 → 词序拼接
...
| 真实缺陷样本全链路复现（用户提供的数电专票 PDF → Rust 文字层 JSON → `extractByCoordinates`） | 修复前 `invoiceType: ""`，修复后 `"专票"` |
