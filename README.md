...
- **三维正交筛选**（v2.5.2，v2.6.3 升级）：类型（专票/普票/车票/通行费/非税/其他）× 格式（PDF/OFD/图片/XML）× 状态（全部/未打印/已打印/重复）自由组合，类型匹配统一走识别单一真源；全选遵循当前筛选——筛选「车票」→全选→2×2 打印，再筛选「专票」→全选→2×1 打印，多步实现分类分批排版；工具栏与筛选区固定在列表顶部不随滚动，筛选区可折叠
